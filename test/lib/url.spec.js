...
const baseUser = new TipaltiBaseUser(1);
...
test('new TipaltiUrl.get() should return urls for production when base user is given', assert => {
  assert.plan(3);

  MockDate.set('1/1/2000', 120);
  const url = new TipaltiUrl(hash, false);
  const { dashboard, invoices, payments } = url.get(baseUser);

  const query =
    '?ts=946677600&payer=payer&idap=1&hashkey=11121cde869844b8760f23c7ed626a2ead92f78f26613669c4a8e0fdf33d5871';
  assert.equals(
    dashboard,
    'https://ui2.tipalti.com/payeedashboard/home' + query
  );
